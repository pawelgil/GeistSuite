...
extension SessionOrchestrator: GeistCamSessionDelegate {
    nonisolated func sessionDidConnect(_ session: GeistCamSession) {}

    nonisolated func sessionDidDisconnect(_ session: GeistCamSession) {
    ...
    nonisolated func session(_ session: GeistCamSession, didActivateSlotWithoutSource slot: CameraSlot) {}

    nonisolated func session(_ session: GeistCamSession, isStreamingChanged isStreaming: Bool) {
        let ref = ObjectIdentifier(session)
        Task { @MainActor [weak self] in
