...
/// Appends log lines to `~/Library/Logs/<bundle-name>/daemon.log`. Mirrors the
...
public enum FileLogSink {
    private static let subdirectory: String = {
        (Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String) ?? "Geist"
    }()

    public static let logsDir: String = {
        (NSHomeDirectory() as NSString)
            .appendingPathComponent("Library/Logs/\(subdirectory)")
    }()
