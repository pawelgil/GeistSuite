...
public actor GeistBroadcastSession {
    ...
    private let videoCapture: BroadcastVideoSource
    private var micAudio: MicSource
    ...
    public init(
        ...
        videoCapture: BroadcastVideoSource = .simulatorScreen,
        micAudio: MicSource = .systemMicrophone,
        ...
        let appShim = try GeistShimBundled.broadcastAppShimDylibPath()
        let extShim = try GeistShimBundled.broadcastExtensionShimDylibPath()
        self.init(
    ...
    public func setMicAudio(_ config: MicSource) {
        micAudio = config
    }
